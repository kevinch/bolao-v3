...
**Action**: Add `display: 'swap'` to IBM_Plex_Sans configuration
**Impact**: Prevents font blocking render, faster perceived load
...
- [ ] Run Lighthouse audit on `/` - expect Performance 85-95+
- [ ] Run Lighthouse on `/bolao/[id]/bet` - expect Performance 85-95+
- [ ] Run Lighthouse on `/bolao/[id]/results` - expect Performance 85-95+
